...
def process_frame(face_landmarks, frame):
    ...
    left_pupil_relative_x = left_pupil_x
    right_pupil_relative_x = right_pupil_x
...
def process_video(video_path, window_size_roc, window_size_ma, std_multiplier=2.0):
    ...
    while cap.isOpened():
        ...
        if results.multi_face_landmarks:
            for face_landmarks in results.multi_face_landmarks:
                ...
                # Testing !!!!!!!!!!!!!!!!!!!!!!!!!!
                left_pupil_relative_x = left_pupil[0]
                right_pupil_relative_x = right_pupil[0]
                ...
                width = frameWidth
                height = frameHeight
                left_pupil_coord = (int(left_pupil[0] * width), int(left_pupil[1] * height) )
                right_pupil_coord = ( int(right_pupil[0] * width), int(right_pupil[1] * height) )

                cv2.drawMarker(frame, left_pupil_coord, (0, 0, 255), cv2.MARKER_CROSS, 10)
                cv2.drawMarker(frame, right_pupil_coord, (0, 255, 0), cv2.MARKER_CROSS, 10)
    ...
    plot_pupil_positions(left_pupil_normalized, right_pupil_normalized, frame_count)

    # Plot pupil positions with moving average
    plot_pupil_positions_ma(left_pupil_relative_x_positions, right_pupil_relative_x_positions,
    ...
    return [left_pupil_rate_of_change, right_pupil_rate_of_change]



def main(video_path = 'error'):

    video_path = r'C:\Users\jared\Documents\IMG_9260.MOV'
    window_size_roc = int(60 * .1)     # Moving average for avg rate of change plot (assuming 60 fps, it is a 10% lag)
    window_size_ma = int(3)     # window size for moving avg chart (default 3)
    std_multiplier = 3  # Change sensitivity for avg rate of change plot (3 default)
...
'''
I think applying a moving average on the rate of change plots would also be helpful
'''